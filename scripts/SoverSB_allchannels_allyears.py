...
def makePlots(inputfile,rerollfile,recoBins,rollingBins,recobw,rb,category):
    ...
    print "length of recoBins ",len(recoBins)
    ...
    sbSlice={} 
    for bn in range(0,histoDict["embedded"].GetNbinsX()):
        sbSlice[bn]=[0,0] 
    ...
    for name in histoDict.keys():
        ...
        for bn in range(0,histo.GetNbinsX()):
            ...
            if (bn%(len(recoBins)-1) in sbbins):
                ...
                #print "skipping bin",bn
                sbSlice[bn][0]=1
                sbSlice[bn][1]=1
                #sbSlice[bn][0]=0
                #sbSlice[bn][1]=0
    print "before integration per slice ",sbSlice
    newslice=0
    ursb=[[]]
    for bn in range(0,histoDict["embedded"].GetNbinsX()):
        #I think this is the problem
        if (bn%(len(recoBins)-1) in sbbins):
            ursb[newslice].append(bn)
        #ursb[newslice].append(bn)
        if ((bn%(len(recoBins)-1)==0)and(bn>1)):
            ursb.append([])
            newslice=newslice+1
    print "The bins that will be rewweighted", ursb
    for setbn in ursb:
        print setbn
        if len(setbn)>1:
            weight_s=0.0
            weight_b=0.0
            for bn in setbn:
                weight_s=weight_s+sbSlice[bn][0]
                weight_b=weight_b+sbSlice[bn][1]
            for bn in setbn:
                sbSlice[bn][0]=weight_s
                sbSlice[bn][1]=weight_b

    #print "The sb weights after integration ",sbSlice
    newslice=0
    for bn in range(0,histoDict["embedded"].GetNbinsX()):
         
        if ((bn%(len(recoBins)-1)==0)and(bn>1)):
            newslice=newslice+1
        if not (bn%(len(recoBins)-1) in sbbins):
            sbSlice[bn][0]=sbSlice[2+newslice*(len(recoBins)-1)][0]
            sbSlice[bn][1]=sbSlice[2+newslice*(len(recoBins)-1)][1]

    print "The sb weights applied ",sbSlice
    #sbSlice["exit"]
    ...
    for name in histoDict.keys():
        ...
        rerolledDict[name]=ROOT.TH1F(category+"_"+name+"_rerolled",category+"_"+name+"_rerolled",len(recoBins),0,len(recoBins)-1)
        ...
        for bn in range(0,histo.GetNbinsX()):
            ...
            #for nbn in range(0,len(recoBins)):
                #reroll condition
                #if ((bn%(len(recoBins))==nbn)):
                #    print sbSlice[bn][0],sbSlice[bn][1]
                #    weight = float(sbSlice[bn][0])/float(sbSlice[bn][0]+sbSlice[bn][1])
                #    rerolledDict[name].SetBinContent(nbn,(rerolledDict[name].GetBinContent(nbn)+histo.GetBinContent(bn))*(weight)/recobw[nbn])
                #    rerolledDict[name].SetBinError(nbn,np.sqrt((rerolledDict[name].GetBinError(nbn))**2+(histo.GetBinError(bn))**2)*(weight)/recobw[nbn])
                #no bin width divide
                if ((bn%(len(recoBins)-1)==nbn)):
                    #print sbSlice[bn][0],sbSlice[bn][1]
                    weight = float(sbSlice[bn][0])/float(sbSlice[bn][0]+sbSlice[bn][1])
                    #weight = 1.0
                    rerolledDict[name].SetBinContent(nbn,(rerolledDict[name].GetBinContent(nbn)+histo.GetBinContent(bn))*(weight))
        ...
        if rb==0:
            rerolledDict[name].Write(name+"_sb")
        if rb==1:
            rerolledDict[name].Write(name)


    #from globalSettings.style import setTDRStyle
    sigstack = ROOT.TH1F("sig","sig",len(recoBins),0,len(recoBins)-1)
    bkgstack = ROOT.TH1F("bkg","bkg",len(recoBins),0,len(recoBins)-1)
    dataclone = ROOT.TH1F("data","data",len(recoBins),0,len(recoBins)-1)
    ...
    for histo in rerolledDict.keys():
        print "working on ",histo
        rerolledRebinned[histo] = ROOT.TH1F(histo+"_rebin",histo+"_rebin",len(recoBins)-1,array('d',recoBins))
        for bn in range(0,rerolledDict[histo].GetNbinsX()):
            #print "scaling histo new value",rerolledDict[histo].GetBinContent(bn),"    times  ",sbweight[bn]
    ...
    #    for bn in range(0,rerolledDict[histo].GetNbinsX()):
    #        #print "scaling histo new value",rerolledDict[histo].GetBinContent(bn),"    times  ",sbweight[bn]
    #        rerolledRebinned[histo].SetBinContent(bn,rerolledDict[histo].GetBinContent(bn))
    #        rerolledRebinned[histo].SetBinError(bn,rerolledDict[histo].GetBinError(bn))
    #        rerolledDict[histo].SetBinContent(bn,rerolledDict[histo].GetBinContent(bn))
    #        rerolledDict[histo].SetBinError(bn,rerolledDict[histo].GetBinError(bn))
    #    rerolledRebinned[histo].Write()
...
def main():
    ...
    mt_0jet_PTH_0_10_2018  = mt_0jet_PTH_0_10_2018_prefit
    mt_0jet_PTH_GE10_2018  = mt_0jet_PTH_GE10_2018_prefit
    mt_boosted_1J_2018     = mt_boosted_1J_2018_prefit
    mt_boosted_GE2J_2018   = mt_boosted_GE2J_2018_prefit
    mt_vbf_PTH_0_200_2018  = mt_vbf_PTH_0_200_2018_prefit
    mt_vbf_PTH_GE_200_2018 = mt_vbf_PTH_GE_200_2018_prefit
    ...
    mt_0jet_PTH_0_10_2018_sb  = mt_0jet_PTH_0_10_2018_subplot
    mt_0jet_PTH_GE10_2018_sb  = mt_0jet_PTH_GE10_2018_subplot
    mt_boosted_1J_2018_sb     = mt_boosted_1J_2018_subplot
    mt_boosted_GE2J_2018_sb   = mt_boosted_GE2J_2018_subplot
    mt_vbf_PTH_0_200_2018_sb  = mt_vbf_PTH_0_200_2018_subplot
    ...
    pad1 = ROOT.TPad("pad1","The pad",0.0016,0.0,1.0,1.0)
    pad2 = ROOT.TPad("pad2","The upper right pad",0.65,0.15,0.9,0.68)
