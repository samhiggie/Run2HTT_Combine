...
int main(int argc, char **argv) 
{
  ...
  //Ele ID efficiency 
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ZL","TTL","VVL","STL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_e_2017","lnN",SystMap<>::init(1.02));

  // Against ele and against mu for real taus
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_t_againstemu_et_2017","lnN",SystMap<>::init(1.01));
  ...
  //cb.cp().process({"ZL","TTL","STL","VVL"}).AddSyst(cb, "CMS_fake_e_2017", "lnN",SystMap<>::init(1.15));    
  cb.cp().process({"TTL","STL","VVL"}).AddSyst(cb, "CMS_fake_e_2017", "lnN",SystMap<>::init(1.15));    
  ...
  cb.cp().process(JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_13TeV_Run2017", "lnN", SystMap<>::init(1.020));
  cb.cp().process(JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_XYfactorization", "lnN", SystMap<>::init(1.008));
  cb.cp().process(JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_lengthScale", "lnN", SystMap<>::init(1.003));
  ...
  cb.cp().process(JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_beamCurrentCalibration", "lnN", SystMap<>::init(1.003));
  cb.cp().process(JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_ghostsAndSatellites", "lnN", SystMap<>::init(1.001));
  ...
  if(not Input.OptionExists("-s"))
    {
      ...
      AddShapesIfNotEmpty({"CMS_singleeletrg_2017","CMS_eletautrg_2017"},
                          JoinStr({ggH_STXS,qqH_STXS,{"VVT","STT","ZT","TTT","VVL","STL","TTL","ZL","WH_htt","ZH_htt","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
                          &cb,
                          1.00,
                          TheFile,CategoryArgs);
      ...
      AddShapesIfNotEmpty({"CMS_scale_fake_e_1prong_2017","CMS_scale_fake_e_1prong1pizero_2017"},
			  {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);
      */
      ...
      AddShapesIfNotEmpty({"CMS_scale_fake_e_1prong_barrel_2017",
	    "CMS_scale_fake_e_1prong1pizero_barrel_2017",
	    "CMS_scale_fake_e_1prong_endcap_2017",
	    "CMS_scale_fake_e_1prong1pizero_endcap_2017"},
			  {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);
      
      AddShapesIfNotEmpty({"CMS_norm_fake_e_slice1_2017",
	    "CMS_norm_fake_e_slice2_2017",
	    "CMS_norm_fake_e_slice3_2017"},
			  {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);
	...
	TheFile,CategoryArgs);
      ...
      AddShapesIfNotEmpty({"CMS_JetEta3to5_2017","CMS_JetEta0to5_2017","CMS_JetRelativeBal_2017",
	    "CMS_JetEta0to3_2017"},
	    JoinStr({ggH_STXS,qqH_STXS,{"ZT","WH_htt","ZH_htt","VVL","STL","ZL","TTL","VVT","STT"}}),
	&cb,
	0.707,
	TheFile,CategoryArgs);            
  ...
  if(not Input.OptionExists("-e"))
    {      
      ...
      cb.cp().process({"embedded"}).AddSyst(cb,"CMS_scale_e_emb_2017","shape",SystMap<>::init(0.866));      
  ...
  if (Input.OptionExists("-c"))
    {
      cb.cp().backgrounds().ExtractShapes(
					  aux_shapes + "et_controls_2017.root",
					  "$BIN/$PROCESS",
