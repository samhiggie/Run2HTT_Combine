...

{
  ...
  vector<string> sig_procs = ch::JoinStr({ggH_STXS,qqH_STXS,{"ZH_htt","WH_htt"}});
  ...
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ZL","TTL","VVL","STL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_e_2016","lnN",SystMap<>::init(1.02));

  // Against ele and against mu for real taus
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_t_againstemu_et_2016","lnN",SystMap<>::init(1.01));

  // Trg efficiency. Can be a single lnN because only single ele trigger
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ZL","TTL","VVL","STL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_singleeletrg_2016","lnN",SystMap<>::init(1.02));
  ...
  //cb.cp().process({"ZL","VVL","TTL","STL"}).AddSyst(cb, "CMS_fake_e_2016 ", "lnN",SystMap<>::init(1.15));  
  cb.cp().process({"VVL","TTL","STL"}).AddSyst(cb, "CMS_fake_e_2016 ", "lnN",SystMap<>::init(1.15));    
  ...
  if(not Input.OptionExists("-s"))
    {
      ...
      AddShapesIfNotEmpty({"CMS_prefiring"},
                          JoinStr({sig_procs,{"VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
                          &cb,
                          1.00,
      ...
      AddShapesIfNotEmpty({"CMS_tauideff_pt30to35_2016","CMS_tauideff_pt35to40_2016","CMS_tauideff_ptgt40_2016"},
			  JoinStr({ggH_STXS,qqH_STXS,{"VVT","STT","ZT","TTT","WH_htt","ZH_htt","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
                          &cb,
                          1.00,
                          TheFile,CategoryArgs);
	...
	std::cout<<"ZLShapes"<<std::endl;
      AddShapesIfNotEmpty({"CMS_scale_fake_e_1prong_2016","CMS_scale_fake_e_1prong1pizero_2016"},
      {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);
      */
      std::cout<<"ZLShapes"<<std::endl;
      AddShapesIfNotEmpty({"CMS_scale_fake_e_1prong_barrel_2016",
	    "CMS_scale_fake_e_1prong1pizero_barrel_2016",
	    "CMS_scale_fake_e_1prong_endcap_2016",
	    "CMS_scale_fake_e_1prong1pizero_endcap_2016"},
			  {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);
      
      AddShapesIfNotEmpty({"CMS_norm_fake_e_slice1_2016",
	    "CMS_norm_fake_e_slice2_2016",
	    "CMS_norm_fake_e_slice3_2016"},
			  {"ZL"},
			  &cb,
			  1.00,
			  TheFile,CategoryArgs);      
	...
	{
	  AddShapesIfNotEmpty({"CMS_htt_boson_reso_met_0jet_2016","CMS_htt_boson_scale_met_0jet_2016"},
			      JoinStr({ggH_STXS,qqH_STXS,{"ZT","ZL","ggH_hww","qqH_hww"}}),
			      &cb,
			      1.00,
			      TheFile,
			      {"et_0jetlow","et_0jethigh"});
	...
	JoinStr({ggH_STXS,qqH_STXS,{"ZT","WH_htt","ZH_htt","VVL","STL","ZL","TTL","TTT","VVT","STT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
	&cb,
	1.000,
	TheFile,CategoryArgs);      
      ...
      AddShapesIfNotEmpty({"CMS_JER_2016"},
			  JoinStr({ggH_STXS,qqH_STXS,{"ZT","VVT","STT","TTT","WH_htt","ZH_htt","VVL","STL","ZL","TTL","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
			  &cb,
			  1.000,
			  TheFile,CategoryArgs);
	...
	JoinStr({ggH_STXS,qqH_STXS,{"ZT","VVT","STT","TTT","ZL","VVL","STL","TTL","WH_htt","ZH_htt","ggH_hww","qqH_hww","WH_hww","ZH_hww"}}),
	&cb,
	1.00,
	TheFile,CategoryArgs);
    }
  ...
  if(not Input.OptionExists("-e"))
    {
      ...
      cb.cp().process({"embedded"}).AddSyst(cb,"CMS_scale_e_emb_2016","shape",SystMap<>::init(0.866));      
