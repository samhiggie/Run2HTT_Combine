...
int main(int argc, char **argv) {
  ...
  TFile* TheFile;
  if(Input.OptionExists("-c")) TheFile = new TFile ((aux_shapes+"em_controls_2018.root").c_str());
  else if (Input.OptionExists("-gf")) TheFile = new TFile((aux_shapes+"smh2018em_GOF.root").c_str());
  else TheFile = new TFile((aux_shapes+"smh2018em.root").c_str());  
  ...
  vector<string> bkg_procs = {"QCD","ZL","VVL","STL","TTL","W","ggH_hww","qqH_hww","WH_hww","ZH_hww"};
  ...
  //Muon ID efficiency
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ZL","TTL","VVL","STL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_m_2018","lnN",SystMap<>::init(1.02));  
  cb.cp().process({"W"}).AddSyst(cb,"CMS_eff_m_2018","lnN",SystMap<>::init(1.01));

  //Electron ID efficiency
  cb.cp().process(JoinStr({{"ZT","TTT","VVT","STT","ZL","TTL","VVL","STL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_eff_e_2018","lnN",SystMap<>::init(1.02));
  ...
  cb.cp().process(JoinStr({{"ZT","VVT","ZL","VVL","ggH_hww","qqH_hww","WH_hww","ZH_hww"},sig_procs})).AddSyst(cb,"CMS_htt_eff_b_2018","lnN",SystMap<>::init(1.005));
  ...
  cb.cp().process(JoinStr({sig_procs,{"W","VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_Run2018", "lnN", SystMap<>::init(1.015));
  cb.cp().process(JoinStr({sig_procs,{"W","VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_XYfactorization", "lnN", SystMap<>::init(1.020));
  cb.cp().process(JoinStr({sig_procs,{"W","VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_lengthScale", "lnN", SystMap<>::init(1.002));
  cb.cp().process(JoinStr({sig_procs,{"W","VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "lumi_beamCurrentCalibration", "lnN", SystMap<>::init(1.002));

  //Trigger uncertainties
  cb.cp().process(JoinStr({sig_procs,{"W","VVL","VVT","STT","STL","ZL","ZT","TTL","TTT","ggH_hww","qqH_hww","WH_hww","ZH_hww"}})).AddSyst(cb, "CMS_trg_emu_2018", "lnN", SystMap<>::init(1.02));
  ...
  if(not Input.OptionExists("-s"))
    {
      ...
      AddShapesIfNotEmpty({"CMS_QCD_njet1_intercept_2018","CMS_QCD_njet1_slope_2018","CMS_QCD_antiiso_2018"},
			  ...
			  TheFile,
			  {"em_boosted1"});
      AddShapesIfNotEmpty({"CMS_QCD_njet2_intercept_2018","CMS_QCD_njet2_slope_2018","CMS_QCD_antiiso_2018"},
			  {"QCD"},
			  &cb,
			  1.00,
			  TheFile,
      ...
      if (Input.OptionExists("-c"))
	{
	  AddShapesIfNotEmpty({"CMS_htt_boson_reso_met_0jet_2018","CMS_htt_boson_scale_met_0jet_2018",
	    ...
	    JoinStr({ggH_STXS,qqH_STXS,{"ZT","ZL","WH_htt","ZH_htt"}}),
	    &cb,
	    1.00,
	    TheFile,CategoryArgs);
	}
	...
	TheFile,CategoryArgs);
      ...
      AddShapesIfNotEmpty({"CMS_JetEta3to5_2018","CMS_JetEta0to5_2018","CMS_JetRelativeBal_2018",
	    "CMS_JetEta0to3_2018"},
	    JoinStr({ggH_STXS,qqH_STXS,{"ZT","WH_htt","ZH_htt","VVL","VVT","STL","ZL","TTL","TTT","STT","W"}}),
	&cb,
	0.707,
	TheFile,CategoryArgs);            
  ...
  if(not Input.OptionExists("-e"))
    {
      ...
      //muon energy scale
      cb.cp().process({"embedded"}).AddSyst(cb,"CMS_scale_emb_m_etalt1p2_2018","shape",SystMap<>::init(0.866));
  ...
  if (Input.OptionExists("-c"))
    {
      cb.cp().backgrounds().ExtractShapes(
					  aux_shapes + "em_controls_2018.root",
					  "$BIN/$PROCESS",
